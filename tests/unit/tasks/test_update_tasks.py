...
from update_tasks.app import lambda_handler, update_task, headers_open
from update_tasks.utils import get_connection, authorized, get_jwt_claims, get_secret


class TestUpdateTasks(unittest.TestCase):

    @patch('update_tasks.app.get_connection')
    @patch('update_tasks.app.authorized')
    ...
    @patch('update_tasks.app.update_task')
    @patch('update_tasks.app.authorized')
    def test_lambda_handler_authorized(self, mock_authorized, mock_update_task):
        event = {
        ...
        mock_update_task.return_value = {
            'statusCode': 200,
            'headers': headers_open,
            'body': json.dumps({'message': 'Task updated successfully with id: 1'})
        }

        response = lambda_handler(event, context)

        self.assertEqual(response['statusCode'], 200)
        self.assertEqual(response['body'], json.dumps({'message': 'Task updated successfully with id: 1'}))
        mock_authorized.assert_called_once_with(event, ["Admins", "Users"])
    ...
    def test_update_task(self, mock_get_connection):
        ...
        response = update_task("1", "Task 1", "Description", "2022-01-01T00:00:00", "2022-01-02T00:00:00", True, False,
                               "1", "1")
        ...
        mock_cursor.execute.assert_called_once_with(
            ...
            ("Task 1", "Description", "2022-01-01T00:00:00", "2022-01-02T00:00:00", True, False, "1", "1", "1")
        )
    ...
    @patch('update_tasks.utils.get_secret')
    ...
    def test_authorized(self):
        ...
        with patch('update_tasks.utils.get_jwt_claims', return_value={'cognito:groups': ['Admins']}):
            self.assertTrue(authorized(event, authorized_groups))

        with patch('update_tasks.utils.get_jwt_claims', return_value={'cognito:groups': ['Users']}):
            self.assertTrue(authorized(event, authorized_groups))

        with patch('update_tasks.utils.get_jwt_claims', return_value={'cognito:groups': ['Guests']}):
            self.assertFalse(authorized(event, authorized_groups))

        with patch('update_tasks.utils.get_jwt_claims', return_value=None):
            self.assertFalse(authorized(event, authorized_groups))
