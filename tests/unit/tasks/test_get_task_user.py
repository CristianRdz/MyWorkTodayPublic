...
from get_task_user.app import lambda_handler, get_tasks, headers_open
from get_task_user.utils import get_connection, authorized, get_jwt_claims, get_secret
from datetime import datetime


class TestGetTaskUser(unittest.TestCase):

    @patch('get_task_user.app.get_connection')
    @patch('get_task_user.app.authorized')
    ...
    @patch('get_task_user.app.get_jwt_claims')
    @patch('get_task_user.app.get_tasks')
    @patch('get_task_user.app.authorized')
    def test_lambda_handler_authorized(self, mock_authorized, mock_get_tasks, mock_get_jwt_claims):
        event = {
        ...
        mock_get_tasks.return_value = {
            ...
            "body": json.dumps([{
                ...
                "date_time_end": "2022-01-01T00:00:00",
                "active": True,
                "finished": False,
                "id_user_assigned": "1",
                "fk_project": "1"
        ...
        mock_get_tasks.assert_called_once_with('user1@example.com')
    ...
    def test_get_tasks(self, mock_get_connection):
        ...
        mock_cursor.fetchall.return_value = [
            ("1", "Task 1", "Description", datetime(2022, 1, 1, 0, 0, 0), datetime(2022, 1, 1, 0, 0, 0), True, False,
             "1", "1")
        ]

        response = get_tasks("user1@example.com")
...
""", ("user1@example.com",))
    ...
    @patch('get_task_user.utils.get_secret')
    ...
    def test_authorized(self):
        ...
        with patch('get_task_user.utils.get_jwt_claims', return_value={'cognito:groups': ['Admins']}):
            self.assertTrue(authorized(event, authorized_groups))

        with patch('get_task_user.utils.get_jwt_claims', return_value={'cognito:groups': ['Users']}):
            self.assertTrue(authorized(event, authorized_groups))

        with patch('get_task_user.utils.get_jwt_claims', return_value={'cognito:groups': ['Guests']}):
            self.assertFalse(authorized(event, authorized_groups))

        with patch('get_task_user.utils.get_jwt_claims', return_value=None):
            self.assertFalse(authorized(event, authorized_groups))
