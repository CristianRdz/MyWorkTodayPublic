...
from insert_projects.app import lambda_handler, insert_project, headers_open
from insert_projects.utils import get_connection, authorized, get_jwt_claims, get_secret

class TestInsertProjects(unittest.TestCase):

    @patch('insert_projects.app.get_connection')
    @patch('insert_projects.app.authorized')
    ...
    @patch('insert_projects.app.insert_project')
    @patch('insert_projects.app.authorized')
    def test_lambda_handler_authorized(self, mock_authorized, mock_insert_project):
        event = {
        ...
        mock_insert_project.return_value = {
            'statusCode': 200,
            'headers': headers_open,
            'body': json.dumps({'message': 'Project inserted successfully with id: 1'})
        }

        response = lambda_handler(event, context)

        self.assertEqual(response['statusCode'], 200)
        self.assertEqual(response['body'], json.dumps({'message': 'Project inserted successfully with id: 1'}))
        mock_authorized.assert_called_once_with(event, ["Admins"])
        mock_insert_project.assert_called_once()

    @patch('insert_projects.app.get_connection')
    def test_insert_project(self, mock_get_connection):
        mock_connection = MagicMock()
        mock_cursor = MagicMock()
        mock_get_connection.return_value = mock_connection
        mock_connection.cursor.return_value = mock_cursor

        response = insert_project("1", "Project 1", "Description", True)

        self.assertEqual(response['statusCode'], 200)
        self.assertEqual(response['body'], json.dumps({'message': 'Project inserted successfully with id: 1'}))
        mock_cursor.execute.assert_called_once_with(
            "INSERT INTO projects (id_project, name_project, description, active) VALUES (%s, %s, %s, %s)",
            ("1", "Project 1", "Description", True)
        )
    ...
    @patch('insert_projects.utils.get_secret')
    ...
    def test_authorized(self):
        ...
        with patch('insert_projects.utils.get_jwt_claims', return_value={'cognito:groups': ['Admins']}):
            self.assertTrue(authorized(event, authorized_groups))

        with patch('insert_projects.utils.get_jwt_claims', return_value={'cognito:groups': ['Users']}):
            self.assertTrue(authorized(event, authorized_groups))

        with patch('insert_projects.utils.get_jwt_claims', return_value={'cognito:groups': ['Guests']}):
            self.assertFalse(authorized(event, authorized_groups))

        with patch('insert_projects.utils.get_jwt_claims', return_value=None):
            self.assertFalse(authorized(event, authorized_groups))
