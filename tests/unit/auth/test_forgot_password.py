...
from forgot_password.app import lambda_handler, get_secret, headers_open
from forgot_password.utils import get_connection, get_jwt_claims, authorized

class TestForgotPassword(unittest.TestCase):

    @patch('forgot_password.app.get_secret')
    @patch('boto3.client')
    def test_lambda_handler(self, mock_boto3_client, mock_get_secret):
        ...
        mock_boto3_client.return_value.forgot_password.return_value = "Success"
        ...
        expected_response = {
            ...
            'statusCode': 200,
            'body': json.dumps({"message": "Confirmation code sent to user's email", "response": "Success"})
        }

        self.assertEqual(response, expected_response)
        mock_boto3_client.assert_called_once_with('cognito-idp', region_name='us-east-1')
        mock_boto3_client.return_value.forgot_password.assert_called_once_with(
    ...
    @patch('forgot_password.utils.get_secret')
    ...
    def test_authorized(self):
        ...
        with patch('forgot_password.utils.get_jwt_claims', return_value={'cognito:groups': ['Admins']}):
            self.assertTrue(authorized(event, authorized_groups))

        with patch('forgot_password.utils.get_jwt_claims', return_value={'cognito:groups': ['Users']}):
            self.assertTrue(authorized(event, authorized_groups))

        with patch('forgot_password.utils.get_jwt_claims', return_value={'cognito:groups': ['Guests']}):
            self.assertFalse(authorized(event, authorized_groups))

        with patch('forgot_password.utils.get_jwt_claims', return_value=None):
            self.assertFalse(authorized(event, authorized_groups))
